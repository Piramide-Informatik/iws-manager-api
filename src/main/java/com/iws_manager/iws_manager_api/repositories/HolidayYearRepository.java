...
public interface HolidayYearRepository extends JpaRepository<HolidayYear, Long> {
        ...
        @Query("SELECT DISTINCT hy FROM HolidayYear hy JOIN hy.publicHoliday ph JOIN ph.stateHolidays sh "
                        + "WHERE hy.date BETWEEN :startDate AND :endDate "
                        + "AND (:stateId IS NULL OR sh.state.id = :stateId) "
                        + "AND sh.isholiday = true")
        List<HolidayYear> findHolidaysBetweenDates(java.time.LocalDate startDate, java.time.LocalDate endDate,
                        Long stateId);
}
